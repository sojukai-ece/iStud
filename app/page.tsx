...
export default function Home() {
  ...
  // Pomodoro State
  const [selectedWorkTime, setSelectedWorkTime] = useState(1500); // Default 25 min
  const [selectedBreakTime, setSelectedBreakTime] = useState(300); // Default 5 min
  ...
  const resetPomodoro = (mode: 'work' | 'break') => {
    ...
    setPomodoroTime(mode === 'work' ? selectedWorkTime : selectedBreakTime);
  };

  const setTimerDuration = (minutes: number) => {
    const seconds = minutes * 60;
    if (pomodoroMode === 'work') {
      setSelectedWorkTime(seconds);
      setPomodoroTime(seconds);
    } else {
      setSelectedBreakTime(seconds);
      setPomodoroTime(seconds);
    }
    setPomodoroIsActive(false);
  };
  ...
  return (
    <div className="min-h-screen text-slate-900 font-sans pb-24 md:pb-10 relative overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-[-1] bg-[#F8FAFC]">
        ...
        <div className="absolute top-[-10%] left-[-10%] w-16040rem] bg-blue-400/20 rounded-full blur-[100px] animate-pulse" style={{ animationDuration: '8s' }}></div>
      ...
      {activeTab === 'study-hub' && (
        <main className="max-w-6xl mx-auto px-4 md:px-6 pt-6 md:pt-10 animate-fade-in relative z-10">
          ...
          {selectedTechnique === 'pomodoro' && activeStudyMode === 'none' ? (
            <div className="bg-white/80 backdrop-blur-xl rounded-[3rem] p-10 md:p-20 shadow-2xl border border-white relative overflow-hidden animate-fade-in flex flex-col items-center">
              ...
              <div className="relative z-10 w-full flex flex-col items-center">
                ...
                <div className="text-8xl sm:text-[12rem] font-black text-[#1B365D] tracking-tighter tabular-nums mb-14 drop-shadow-sm text-center">
                  {formatTime(pomodoroTime)}
                </div>
                
                <div className="flex flex-col gap-8 w-full max-w-lg">
                  {/* Mode Selector */}
                  <div className="flex gap-2 bg-slate-100/80 backdrop-blur-md p-2 rounded-full border border-slate-200 w-full">
                    <button onClick={() => resetPomodoro('work')} className={`flex-1 py-3 rounded-full font-bold text-sm transition-all ${pomodoroMode === 'work' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}>Work Mode</button>
                    <button onClick={() => resetPomodoro('break')} className={`flex-1 py-3 rounded-full font-bold text-sm transition-all ${pomodoroMode === 'break' ? 'bg-white shadow-sm text-emerald-600' : 'text-slate-500 hover:text-slate-700'}`}>Break Mode</button>
                  </div>

                  {/* Play/Pause Button */}
                  <button onClick={togglePomodoro} className={`w-full py-5 rounded-full font-black text-xl shadow-xl transition-all duration-300 hover:scale-105 ${pomodoroIsActive ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' : 'bg-[#1B365D] text-white hover:shadow-blue-900/30'}`}>
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="flex gap-2">
                      {pomodoroMode === 'work' ? (
                        [25, 35, 50].map(mins => (
                          <button key={mins} onClick={() => setTimerDuration(mins)} className={`px-5 py-2.5 rounded-2xl text-xs font-black transition-all ${selectedWorkTime === mins * 60 ? 'bg-indigo-100 text-indigo-700 border-indigo-200 shadow-sm scale-105' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'} border`}>{mins} min</button>
                        ))
                      ) : (
                        [5, 15, 30].map(mins => (
                          <button key={mins} onClick={() => setTimerDuration(mins)} className={`px-5 py-2.5 rounded-2xl text-xs font-black transition-all ${selectedBreakTime === mins * 60 ? 'bg-emerald-100 text-emerald-700 border-emerald-200 shadow-sm scale-105' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'} border`}>{mins} min</button>
                        ))
                      )}
                    </div>
                  </div>
              ...
              {selectedFolder && activeStudyMode === 'active-recall' && (
                <div className="max-w-4xl mx-auto animate-fade-in relative z-10">
                  ...
                  {currentCardIndex >= flashcards.length ? (
                    ...
                    <div className="bg-white/90 backdrop-blur-xl rounded-[3rem] min-h-125 flex flex-col justify-center p-8 md:p-12 shadow-2xl border border-white relative overflow-hidden group">
                  ...
                  {!blurtingFinished ? (
                    <div className="bg-white/90 backdrop-blur-xl rounded-[3rem] p-8 md:p-12 shadow-2xl border border-white flex flex-col">
                      ...
                      <textarea 
                        ...
                        className="w-full h-125 bg-amber-50/50 border-2 border-slate-200 rounded-4xl8 text-slate-800 text-lg font-medium leading-relaxed focus:outline-none focus:border-amber-400 resize-none disabled:opacity-70 transition-all shadow-inner"
                      />
                    </div>
                  ) : (
