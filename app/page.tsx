...
export default function Home() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#F8FAFC] text-slate-900 font-sans overflow-hidden">
      ...
      {/* --- MOBILE TOP BAR --- */}
      <div className="md:hidden flex items-center justify-between bg-white border-b border-slate-200 px-5 py-4 shrink-0 z-40">
        ...
        {user ? (
          <button onClick={() => setIsSettingsOpen(true)} className="flex items-center">
            {user.user_metadata?.avatar_url ? (
              ...
              <div className="w-8 h-8 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center text-xs font-bold border border-slate-200 shadow-sm">{user.email?.charAt(0).toUpperCase()}</div>
            )}
          </button>
        ) : (
           <Link href="/login" className="text-sm font-bold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg">Sign In</Link>
        )}
      </div>
      ...
      <aside className="hidden md:flex flex-col w-65 bg-white border-r border-slate-200 h-full fixed left-0 top-0 z-40 overflow-y-auto custom-scrollbar">
        ...
        <nav className="px-4 py-2 flex flex-col gap-1">
          <button onClick={() => { setActiveTab('dashboard'); setSelectedFolder(null); }} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl font-bold text-sm transition-colors ${activeTab === 'dashboard' ? 'bg-slate-100 text-[#0F172A]' : 'text-slate-600 hover:bg-slate-50'}`}>
            <span className="text-lg">🏠</span> Home
          </button>
          <button onClick={() => { setActiveTab('focus-hub'); setSelectedFolder(null); }} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl font-bold text-sm transition-colors ${activeTab === 'focus-hub' ? 'bg-slate-100 text-[#0F172A]' : 'text-slate-600 hover:bg-slate-50'}`}>
            <span className="text-lg">⏱️</span> Focus Hub
          </button>
          <button onClick={() => { setActiveTab('decks'); setSelectedFolder(null); }} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl font-bold text-sm transition-colors ${activeTab === 'decks' ? 'bg-slate-100 text-[#0F172A]' : 'text-slate-600 hover:bg-slate-50'}`}>
          ...
          {user ? (
            <button onClick={() => setIsSettingsOpen(true)} className="flex items-center gap-3 px-4 py-2.5 rounded-xl font-bold text-sm transition-colors text-slate-600 hover:bg-slate-50 relative mt-1">
              ...
              Profile
              <span className="absolute right-4 top-2.5 bg-red-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full">{activeStreak}</span>
            </button>
          ) : (
        ...
        <div className="px-4 py-4 flex flex-col gap-2 border-b border-slate-100">
          ...
          <button onClick={() => { setActiveTab('decks'); setIsAddingFolder(!isAddingFolder); }} className="w-full bg-white border border-slate-200 text-[#0F172A] font-bold py-2.5 rounded-2xl flex items-center justify-center gap-2 hover:bg-slate-50 transition-colors">
            ＋ Add
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="flex justify-between items-center mb-3 px-2">
            ...
            <span onClick={() => { setActiveTab('decks'); setIsAddingFolder(true); }} className="text-lg cursor-pointer text-slate-400 hover:text-slate-700">＋</span>
          </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-50 px-2 py-2 flex justify-around items-center">
        {[{ id: 'dashboard', icon: '🏠', label: 'Home' }, { id: 'focus-hub', icon: '⏱️', label: 'Focus' }, { id: 'decks', icon: '📁', label: 'Decks', action: () => setSelectedFolder(null) }, { id: 'community', icon: '🌍', label: 'Chat' }].map((tab, i) => (
      ...
      <main className="flex-1 md:ml-65 h-full overflow-y-auto pb-20 md:pb-8 relative">
          ...
          <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-12 animate-fade-in space-y-8 md:space-y-10">
            ...
            <div className="bg-white rounded-3xl md:rounded-4xl p-6 md:p-8 border border-slate-200 shadow-sm flex flex-col xl:flex-row gap-6 md:gap-8 items-center justify-between relative overflow-hidden">
              ...
              <div className="flex-1 z-10 w-full">
                ...
                <p className="text-slate-600 font-medium italic text-base md:text-lg border-l-4 border-blue-500 pl-4 py-1">
                  "{dailyQuote}"
                </p>
              </div>

              <div className="flex flex-col sm:flex-row w-full xl:w-auto gap-4 z-10">
                <div className="flex flex-1 items-center gap-4 bg-orange-50 border border-orange-100 p-4 md:p-5 rounded-2xl md:rounded-3xl shadow-inner justify-center md:justify-start">
                  ...
                  <div>
                    ...
                    <p className="text-xl md:text-2xl font-black text-orange-600">{activeStreak} {activeStreak === 1 ? 'Day' : 'Days'}</p>
                  </div>
                </div>
                <div className={`flex flex-1 items-center gap-4 ${getAcademicRank().bg} border border-slate-100 p-4 md:p-5 rounded-2xl md:rounded-3xl shadow-inner justify-center md:justify-start`}>
                  <div className="text-4xl md:text-5xl drop-shadow-sm">{getAcademicRank().icon}</div>
                  <div>
                    <p className={`text-[10px] font-extrabold ${getAcademicRank().color} opacity-80 uppercase tracking-widest mb-1`}>Current Rank</p>
                    <p className={`text-xl font-black ${getAcademicRank().color} leading-tight`}>{getAcademicRank().title}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* VIEW 1.5: DECKS TAB (Moved from Dashboard) */}
        {activeTab === 'decks' && !selectedFolder && (
          <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-12 animate-fade-in">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setIsAddingFolder(!isAddingFolder)} className="bg-[#0F172A] text-white px-4 py-2 rounded-xl font-bold text-sm shadow-sm hover:bg-slate-800">＋ New Deck</button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {folders.length === 0 ? (
                  <div className="col-span-full bg-white border-2 border-dashed border-slate-200 rounded-2xl md:rounded-3xl p-10 text-center text-slate-500 font-bold">
                    No decks created yet. Click the + above to start studying!
                  </div>
              ) : (
                folders.map((folder, idx) => (
                  <div key={folder.id} onClick={() => openFolder(folder)} className="bg-white rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden p-6 pl-8 cursor-pointer hover:shadow-md hover:-translate-y-1 hover:border-blue-200 transition-all group flex flex-col justify-between">
                    <div className={`absolute left-0 top-0 bottom-0 w-3 ${getDeckColor(idx)}`}></div>
                    <div className="mb-4">
                      <h3 className="font-black text-[#0F172A] text-lg group-hover:text-blue-600 transition-colors line-clamp-2">{folder.name}</h3>
                    ...
                    <div className="flex justify-between items-center mt-auto border-t border-slate-50 pt-4">
                      <span className="text-xs font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded-md">Ready to study</span>
                      <span className="text-slate-300 group-hover:text-blue-500 font-bold text-xl transition-colors">→</span>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 md:py-12 animate-fade-in">
             ...
             <div className="bg-white p-5 md:p-8 rounded-3xl md:rounded-4xl border border-slate-200 shadow-sm mb-6 md:mb-8 relative">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-5 md:pb-6 mb-5 md:mb-6 pr-10">
                  <div>
                    <h2 className="text-2xl md:text-3xl font-black text-[#0F172A]">{selectedFolder.name}</h2>
                    <p className="text-sm md:text-base text-slate-500 font-medium mt-1">{flashcards.length} cards in this deck</p>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto mt-4 md:mt-0">
                    ...
                    <button onClick={launchStudyMode} disabled={flashcards.length === 0} className="bg-[#0F172A] text-white font-bold text-sm px-6 py-3 rounded-xl shadow-md hover:bg-slate-800 disabled:opacity-50 w-full sm:w-auto">
                      Launch 🚀
                    </button>
                  </div>
                </div>
             ...
             <div className="space-y-3">
                ...
                {flashcards.map((card) => (
                  <div key={card.id} className="bg-white p-4 md:p-5 rounded-xl md:rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-2 md:gap-4 relative group">
                    ...
                    {/* Card Delete Button */}
                    <button onClick={() => handleDeleteCard(card.id)} className="absolute top-4 right-4 sm:top-auto sm:bottom-4 sm:right-5 text-slate-300 hover:text-red-500 transition-colors opacity-100 sm:opacity-0 group-hover:opacity-100 text-lg" title="Delete Card">🗑️</button>
                  </div>
                ))}
          ...
          <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-12 animate-fade-in flex flex-col lg:flex-row gap-6 md:gap-10">
            ...
            <div className="flex-1 bg-white rounded-4xl p-6 md:p-12 border border-slate-200 shadow-sm flex flex-col items-center justify-center relative overflow-hidden">
              ...
              <div className="relative z-10 w-full flex flex-col items-center">
                ...
                <div className="flex gap-2 bg-slate-100 p-1.5 rounded-full border border-slate-200 w-full max-w-sm mb-10">
                  <button onClick={() => { setPomodoroIsActive(false); setPomodoroMode('work'); setPomodoroTime(selectedWorkTime); }} className={`flex-1 py-2.5 rounded-full font-bold text-xs md:text-sm transition-all ${pomodoroMode === 'work' ? 'bg-white shadow-sm text-[#0F172A]' : 'text-slate-500 hover:text-slate-700'}`}>Focus</button>
                  <button onClick={() => { setPomodoroIsActive(false); setPomodoroMode('break'); setPomodoroTime(selectedBreakTime); }} className={`flex-1 py-2.5 rounded-full font-bold text-xs md:text-sm transition-all ${pomodoroMode === 'break' ? 'bg-white shadow-sm text-[#0F172A]' : 'text-slate-500 hover:text-slate-700'}`}>Break</button>
                </div>

                <div className="text-[6rem] md:text-[8rem] font-black text-[#0F172A] tracking-tighter tabular-nums mb-10 leading-none">
                  {formatClock(pomodoroTime)}
                </div>

                <button onClick={() => setPomodoroIsActive(!pomodoroIsActive)} className={`w-full max-w-sm py-4 rounded-2xl font-black text-lg transition-all duration-300 ${pomodoroIsActive ? 'bg-red-50 text-red-600 border border-red-200 hover:bg-red-100' : 'bg-[#0F172A] text-white hover:bg-slate-800 shadow-md'}`}>
            ...
            <div className="w-full lg:w-100 flex flex-col h-125 lg:h-auto bg-white rounded-4xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="flex-1 p-6 overflow-y-auto space-y-3 custom-scrollbar bg-white">
                {tasks.length === 0 ? (
                  <p className="text-center text-slate-400 font-bold mt-10 text-sm">Add tasks to focus on.</p>
                ) : (
                  tasks.map(task => (
                    <div key={task.id} className={`flex items-center justify-between p-3 rounded-xl border ${task.completed ? 'bg-slate-50 border-slate-100' : 'bg-white border-slate-200 shadow-sm'} transition-colors group`}>
                      ...
                      <button onClick={() => deleteTask(task.id)} className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-red-500 transition-opacity font-black text-xs px-2">✕</button>
                    </div>
                  ))
                )}
              </div>

              <form onSubmit={handleAddTask} className="p-4 border-t border-slate-100 bg-slate-50 shrink-0">
