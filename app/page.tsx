                      ...
                      <div key={msg.id} className={`flex items-end gap-2 md:gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'} animate-fade-in`}>
                        {!isMe && (
                          msg.avatar_url ? (
                            ...
                            <div className="w-8 h-8 rounded-full bg-slate-300 text-white flex items-center justify-center text-[10px] font-bold shrink-0 shadow-sm">{msg.user_name.charAt(0).toUpperCase()}</div>
                          )
                        )}
