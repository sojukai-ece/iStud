      ...
      <main className="flex-1 md:ml-[260px] h-full overflow-y-auto pb-20 md:pb-8 relative">
        ...
        {activeTab === 'dashboard' && !selectedFolder && (
          <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-12 animate-fade-in space-y-8 md:space-y-10">
            ...
            <div className="bg-white rounded-3xl md:rounded-[2rem] p-6 md:p-8 border border-slate-200 shadow-sm flex flex-col md:flex-row gap-6 md:gap-8 items-center justify-between relative overflow-hidden">
              ...
              <div className="flex items-center gap-4 bg-orange-50 border border-orange-100 p-4 md:p-5 rounded-2xl md:rounded-3xl shrink-0 shadow-inner z-10 w-full md:w-auto justify-center md:justify-start">
                ...
                <div>
                  ...
                  <p className="text-xl md:text-2xl font-black text-orange-600">3 Days</p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              {/* Add Folder Input */}
              {isAddingFolder && (
                <form onSubmit={handleAddFolder} className="mb-4 md:mb-6 flex flex-col sm:flex-row gap-2 md:gap-3 animate-fade-in">
                  ...
                  <button type="submit" className="bg-[#0F172A] text-white px-6 py-3 rounded-xl font-bold text-sm shadow-sm w-full sm:w-auto">Create</button>
                </form>
              )}
              ...
              <div className="flex flex-col gap-3 md:gap-4">
                {folders.length === 0 ? (
                   <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl md:rounded-3xl p-8 md:p-10 text-center text-slate-500 font-bold text-sm md:text-base">
                     No decks created yet. Click the + above to start studying!
                   </div>
                ) : (
                  folders.map((folder, idx) => (
                    <div key={folder.id} onClick={() => openFolder(folder)} className="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden p-4 md:p-5 pl-6 md:pl-7 cursor-pointer hover:shadow-md hover:border-blue-200 transition-all group flex justify-between items-center">
                      <div className={`absolute left-0 top-0 bottom-0 w-2 md:w-3 ${getDeckColor(idx)}`}></div>
                      <div>
                        <h3 className="font-bold text-[#0F172A] text-sm md:text-base group-hover:text-blue-600 transition-colors">{folder.name}</h3>
                        <p className="text-xs md:text-sm font-medium text-slate-500 mt-0.5 md:mt-1">Custom Deck</p>
                      </div>
                      <span className="text-slate-300 group-hover:text-slate-500 font-bold text-lg md:text-xl transition-colors">→</span>
                    </div>
                  ))
                )}
              </div>
        ...
        {activeTab === 'dashboard' && selectedFolder && activeStudyMode === 'none' && (
          <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 md:py-12 animate-fade-in">
             <button onClick={() => setSelectedFolder(null)} className="text-xs md:text-sm font-bold text-slate-500 hover:text-[#0F172A] flex items-center gap-2 mb-4 md:mb-6">← Back to My Decks</button>
             
             <div className="bg-white p-5 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 shadow-sm mb-6 md:mb-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-5 md:pb-6 mb-5 md:mb-6">
                  <div>
             ...
             <div className="space-y-3">
                ...
                {flashcards.map((card) => (
                  <div key={card.id} className="bg-white p-4 md:p-5 rounded-xl md:rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-2 md:gap-4">
                    <div className="flex-1 pb-3 sm:pb-0 sm:border-r border-slate-100 sm:pr-4">
